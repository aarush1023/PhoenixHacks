...
def predict(landmarks):
    print(len(landmarks))
    return model.predict(np.array(landmarks).reshape(1, -1))
...
while True:
    ...
    if started:
        if HandLandmarkerResult.hand_landmarks != []:
            count += 1
            for landmark in HandLandmarkerResult.hand_landmarks[0]:
    ...
    temp_image = cv2.putText(temp_image, "Prediction: "+text, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
